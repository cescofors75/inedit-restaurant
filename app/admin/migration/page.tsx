...
type ItemType = 'categories' | 'subcategories' | 'items';
...
export default function MigrationPage() {
  ...
  const fetchDatabaseInfo = async () => {
    try {
      ...
      const tables = ['beverage_categories', 'beverage_subcategories', 'beverage_items'];
      const tableData = [];
  ...
  // Función para cargar el archivo JSON desde public/data/beverages.json
  const loadJsonFile = async () => {
    setIsLoading(true);
    addLog('info', 'Iniciando carga del archivo JSON desde public/data/beverages.json...');
    ...
    try {
      ...
      const totalItems = 
        ...
        // Para subcategorías, estimamos basándonos en los items
        (new Set(data.items?.map((item: any) => item.subcategoryId).filter(Boolean)).size || 0) +
        (data.items?.length || 0);
  ...
  // Función para migrar subcategorías
  const migrateSubcategories = async () => {
    const { items } = jsonData;
    
    if (!items || !Array.isArray(items)) {
      addLog('error', 'No se encontraron items válidos para extraer subcategorías');
      return;
    }
    
    // Extraer subcategorías únicas de los items
    const subcategoriesMap = new Map();
    
    for (const item of items) {
      if (item.subcategoryId && item.categoryId && !subcategoriesMap.has(item.subcategoryId)) {
        subcategoriesMap.set(item.subcategoryId, {
    ...
    if (subcategoriesArray.length === 0) {
      addLog('warning', 'No se encontraron subcategorías para migrar');
      return;
    }
    ...
    for (const subcategory of subcategoriesArray) {
      try {
        ...
        successCount++;
        addLog('success', `Subcategoría migrada: ${subcategory.id}`);
      } catch (error) {
        errorCount++;
        addLog('error', `Error al migrar subcategoría ${subcategory.id}: ${error instanceof Error ? error.message : String(error)}`);
      } finally {
        // Actualizar el estado de la migración
        setMigrationStatus(prev => ({
          ...prev,
    ...
    addLog('info', `Migración de subcategorías completada: ${successCount} exitosas, ${errorCount} errores`);
  };
  ...
  const migrateProducts = async () => {
    ...
    for (const item of items) {
      try {
        const {
          id, name, description, price, categoryId, subcategoryId,
          region, grapes, image, priceGlass, priceShot
        } = item;
        ...
        const { data, error } = await supabase
          ...
          .insert({
            ...
            price_shot: priceShot ? parseFloat(priceShot) : null,
  ...
  const startMigration = async () => {
    ...
    setMigrationStatus({
      total: jsonData ? 
        ...
        (new Set(jsonData.items?.map((item: any) => item.subcategoryId).filter(Boolean)).size || 0) +
    ...
    try {
      // Migrar en orden: categorías -> subcategorías -> items
      await migrateItems('categories');
      await migrateItems('subcategories');
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10">
          <h1 className="text-3xl font-bold text-gray-900 sm:text-4xl">
            Migración de Datos de Bebidas
          </h1>
          <p className="mt-3 max-w-2xl mx-auto text-lg text-gray-500">
            Herramienta para migrar los datos desde <code className="bg-gray-100 px-1 py-0.5 rounded">public/data/beverages.json</code> a tablas relacionales en Supabase
          </p>
        </div>
